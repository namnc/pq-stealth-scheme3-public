...
- **No conformance runner.** The specification's Test Cases section requires that a third party
  ...
  check.
- **No sibling rungs.** The ladder this rung belongs to has others — a KEM-only per-payment
  variant, a pairwise-channel pair, and a post-quantum *spending* rung. None of their
  specifications ship here, which is the point of this tree.
  The KEM-only variant's code does ship, inside `crates/per-payment`, because §2.8 requires the
